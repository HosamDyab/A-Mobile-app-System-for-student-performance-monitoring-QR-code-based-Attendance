...
class StudentSearchCubit extends Cubit<StudentSearchState> {
  ...
  Future<void> loadAllCourses() async {
    ...
    try {
      ...
      final safeCourses = (courses ?? [])
          .where((c) => c.title.trim().isNotEmpty && c.title != 'Unknown Course')
          .toList();
  ...
  Future<void> searchCourses(String query) async {
    if (!hasStudentId) {
      emit(state.copyWith(isLoadingCourses: false, error: 'Student ID not set'));
      return;
    }
  ...
  Future<void> loadAllFaculty() async {
    if (!hasStudentId) {
      emit(state.copyWith(isLoadingFaculty: false, error: 'Student ID not set'));
      return;
    }
    ...
    try {
      ...
      final safeFaculty = (faculty ?? [])
          .where((f) => f.fullName.trim().isNotEmpty && f.fullName != 'Unknown Faculty')
          .toList();
