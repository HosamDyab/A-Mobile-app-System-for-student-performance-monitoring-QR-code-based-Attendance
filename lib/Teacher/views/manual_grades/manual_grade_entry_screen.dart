  ...
  Future<void> _submitGrades() async {
    ...
    try {
      ...
      for (var student in _filteredStudents) {
        ...
        // Calculate Year_Work from all year work components
        double yearWork = 0;
        yearWork += double.tryParse(controllers['Quiz1']?.text ?? '0') ?? 0;
        yearWork += double.tryParse(controllers['Quiz2']?.text ?? '0') ?? 0;
        yearWork += double.tryParse(controllers['Quiz3']?.text ?? '0') ?? 0;
        yearWork +=
            double.tryParse(controllers['Assignment1']?.text ?? '0') ?? 0;
        yearWork +=
            double.tryParse(controllers['Assignment2']?.text ?? '0') ?? 0;
        yearWork +=
            double.tryParse(controllers['Assignment3']?.text ?? '0') ?? 0;
        yearWork += double.tryParse(controllers['Lab']?.text ?? '0') ?? 0;
        yearWork +=
            double.tryParse(controllers['MiniProject']?.text ?? '0') ?? 0;
      ...
      // Use LectureGrade table instead of StudentGrade
      await _supabase.from('LectureGrade').upsert(
            gradeUpdates,
